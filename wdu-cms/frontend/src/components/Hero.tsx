...
export default function Hero() {
  ...
  useEffect(() => {
    const loadSettings = () => {
      // 1. Prioritas Utama: Data dari Editor Baru (wdu_admin_sections)
      ...
      if (storedSections) {
        try {
          ...
          if (heroSection?.content && heroSection.status === 'Published') {
            const { badge, headline, description, bgImage } = heroSection.content;
            setHeroData((prev) => ({
              ...prev,
              badgeText: badge || prev.badgeText,
              // Pecah headline berdasarkan baris baru (enter)
              headline: headline ? headline.split('\n').filter((l: string) => l.trim() !== '') : prev.headline,
              description: description || prev.description,
              // Update image pertama jika ada bgImage
              carouselImages: bgImage ? [bgImage, ...prev.carouselImages.slice(1)] : prev.carouselImages
            }));
            return; // Berhenti jika data baru ditemukan
          }
        } catch (e) {
      ...
      // 2. Fallback: Data dari Editor Lama (Legacy)
      const storedLegacy = localStorage.getItem('wdu_admin_settings');
      if (storedLegacy) {
        try {
          const parsed = JSON.parse(storedLegacy);
          if (parsed?.heroes?.home) {
            const homeData = parsed.heroes.home;
            setHeroData((prev) => ({ 
              ...prev, 
              badgeText: homeData.badgeText || prev.badgeText,
              headline: homeData.headline || prev.headline,
              description: homeData.description || prev.description,
              ctaText: homeData.ctaText || prev.ctaText,
              ctaLink: homeData.ctaLink || prev.ctaLink
            }));
          }
        } catch (e) {
      ...
      >
        {heroData.carouselImages.map((_, i) => (
          <motion.div
            ...
            className={`h-1.5 cursor-pointer rounded-full bg-white/30 transition-all duration-500 ${currentIndex === i ? 'w-12 bg-primary' : 'w-4 hover:w-6 hover:bg-white/50'
              }`}
