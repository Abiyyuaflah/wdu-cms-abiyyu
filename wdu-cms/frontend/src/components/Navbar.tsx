...
export default function Navbar() {
  ...
  // Detect file extension and return appropriate filename for download
  const getDownloadFilename = (url: string) => {
    ...
    return 'Company-Profile.pdf';
  };

  // Handle download with JavaScript (fix cross-origin issues)
  const handleDownload = async (e: React.MouseEvent) => {
    e.preventDefault();
    
    if (!config.companyProfile) {
      alert('Belum ada file Company Profile yang diupload');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(config.companyProfile, { headers });
      ...
      link.href = url;
      link.download = getDownloadFilename(config.companyProfile) || 'Company-Profile.pdf';
      ...
      alert('Gagal mendownload file: ' + (err.message || 'Terjadi kesalahan'));
    }
  };
