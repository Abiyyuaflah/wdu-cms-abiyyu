    ...
    const loadAll = () => {
      ...
      if (storedSections) {
        try {
          ...
          const clientSection = homeSections.find((s: any) => s.type === 'TrustClients');
          
          if (clientSection?.content && clientSection.status === 'Published') {
            const { title, description, partners } = clientSection.content;
            setSectionData({
              title: title || sectionData.title,
              description: description || sectionData.description
            });
            if (partners && partners.length > 0) {
              setLogos(partners.map((p: any) => ({ name: p.name, url: p.logoUrl })));
              return;
            }
          }
        } catch (e) {}
