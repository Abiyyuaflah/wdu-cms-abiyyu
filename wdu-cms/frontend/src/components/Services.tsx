...
export default function Services() {
    ...
    const loadAll = () => {
      ...
      if (storedSections) {
        try {
          ...
          const servicesSection = homeSections.find((s: any) => s.type === 'Services');
          
          if (servicesSection?.content && servicesSection.status === 'Published') {
            setServicesContent({
              title: servicesSection.content.title || 'Solusi terbaik untuk Anda!',
              description: servicesSection.content.description || ''
            });

            if (servicesSection.content.items && servicesSection.content.items.length > 0) {
              const mapped = servicesSection.content.items.map((item: any) => ({
                title: item.title,
  ...
  const displayServices = cmsServices.length > 0 ? cmsServices : services;
  
  const activeServices = displayServices.filter(service => {
